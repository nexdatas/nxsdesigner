...
# dialog defining a slabs tag
class SlabsDlg(QDialog):

    # constructor
    # \param parent patent instance
    def __init__(self, parent=None):
        super(SlabsDlg, self).__init__(parent)
    ...
    # \brief It copies the slabs name and type from lineEdit widgets
...
#    form.lengths = [None, 3]
    ...
    if form.result():
        if form.rank:
            logger.info("Slabs: rank = %s" % (form.rank))
        if form.lengths:
            logger.info("Lengths:")
