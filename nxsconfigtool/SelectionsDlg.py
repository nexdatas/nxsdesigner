...
class SelectionsDlg(QDialog):
    ...
    def createGUI(self):
        ...
        if self.keytype.lower() == "slices":
            self.keytype = "slices"
            self.ui.keytypeComboBox.setValue(0)
        else:
            self.keytype = "slab"
            self.ui.keytypeComboBox.setValue(1)
        try:
    ...
    def __tableItemChanged(self, item):
        ...
        if row not in range(len(self.lengths)):
            return
        column = self.ui.selTableWidget.currentColumn()
        if column == 0:
            try:
                if item.text():
        ...
        for elem in  elems:
            for row, ln in enumerate(elem):
                ...
                if selectedDim is not None and selectedDim == row:
                    selected = item
                self.ui.selTableWidget.setItem(row, 0, item)
...
if __name__ == "__main__":
    ...
    form.rank = 2
    form.keytype = "slides"
    form.lengths = [[2,10, None], [None,20, 2]]
#    form.lengths = [None, 3]
    form.createGUI()
    form.show()
    app.exec_()
