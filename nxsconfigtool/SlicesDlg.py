...
# dialog defining a slices tag
class SlicesDlg(QDialog):

    # constructor
    # \param parent patent instance
    def __init__(self, parent=None):
        super(SlicesDlg, self).__init__(parent)
    ...
    # \brief It copies the slices name and type from lineEdit widgets
...
#    form.lengths = [None, 3]
    ...
    if form.result():
        if form.rank:
            logger.info("Slices: rank = %s" % (form.rank))
        if form.lengths:
            logger.info("Lengths:")
