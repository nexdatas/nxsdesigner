...
docker exec  ndts /bin/bash -c 'source .sh.sh'
docker exec  ndts /bin/bash -c 'ls -ltr '
docker exec  ndts /bin/bash -c 'source .sh.sh ; pixi add  numpy pytango  setuptools pip wheel argcomplete lxml pytz pyyaml pytango python-dateutil pninexus fabio h5py matplotlib-base blissdata pytest docutils nxsconfigserver  pip pymysql nxstools qt-main pyqt>=5.15,<6 '
